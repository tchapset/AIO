...
async function getOrCreateUser(id) {
  try {
    ...
    try {
      ...
      await client.query(
        `INSERT INTO users (user_id, username, main_balance, referral_code, plans) 
         VALUES ($1, $2, 0, $3, '{}')`,
        [id, 'user' + id, referralCode]
      );
    ...
    if (updates.referral_balance !== undefined || updates.trading_balance !== undefined) {
      ...
      if (updates.referral_balance !== undefined) {
        ...
        if (difference > 0) {
          ...
          // (par exemple 0.000001 SOL pour montrer que le compte est actif)
...
async function showMyEarnings(chatId, messageId) {
  try {
    ...
    // Message HTML simple
    const message = 
      `<b>📈 VOS GAINS DÉTAILLÉS</b>\n\n` +
      `<b>💰 TOTAUX DEPUIS LE DÉBUT:</b>\n` +
      `• 🤖 <b>Trading:</b> ${lifetimeEarnings.trading.toFixed(6)} SOL ($${convertSOLtoUSDT(lifetimeEarnings.trading).toFixed(2)})\n` +
      `• 👥 <b>Parrainage:</b> ${lifetimeEarnings.referral.toFixed(6)} SOL ($${convertSOLtoUSDT(lifetimeEarnings.referral).toFixed(2)})\n` +
      `• 🏦 <b>Total gagné:</b> ${lifetimeEarnings.total.toFixed(6)} SOL ($${convertSOLtoUSDT(lifetimeEarnings.total).toFixed(2)})\n\n` +
      `<b>💵 DISPONIBLE MAINTENANT:</b> ${(parseFloat(user.main_balance) || 0).toFixed(6)} SOL ($${convertSOLtoUSDT(parseFloat(user.main_balance) || 0).toFixed(2)})\n\n` +
      `<b>💡 Tous vos gains sont automatiquement dans votre compte principal!</b>`;
    ...
    console.error('showMyEarnings error:', error.message);
  }
}
...
async function startTrading(chatId, messageId) {
  try {
    ...
    await addTransaction(chatId, 'daily_earning', totalGain, `Trading réussi - ${activePlans.length} plan(s) actif(s)`);
